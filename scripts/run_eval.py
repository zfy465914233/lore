...
BENCHMARK_CASES: list[BenchmarkCase] = [
    ...
    BenchmarkCase(
        ...
        expected_top_evidence_ids=["markov-chain-definition"],
        min_citations=1,
    ),
    BenchmarkCase(
        query="What is the duality theorem in linear programming?",
        category="definition",
        expected_route="local-led",
        expected_top_evidence_ids=["lp-duality-theorem"],
        min_citations=1,
    ),
    BenchmarkCase(
        ...
        category="definition",
        expected_route="local-led",
        expected_top_evidence_ids=["quantization-aware-training-method"],
        min_citations=1,
    ),
    # --- Derivation queries (local-led) ---
    BenchmarkCase(
        query="Show the derivation for the stationary distribution of a Markov chain.",
        category="derivation",
        expected_route="local-led",
        expected_top_evidence_ids=["stationary-distribution-derivation"],
        min_citations=1,
    ),
    BenchmarkCase(
        query="Derive the QPE error bound.",
        category="derivation",
        expected_route="local-led",
        expected_top_evidence_ids=["qpe-error-bound-derivation"],
        min_citations=1,
    ),
    # --- Freshness queries (web-led) ---
    BenchmarkCase(
        query="What is the latest SOTA method for quantization-aware training?",
        category="freshness",

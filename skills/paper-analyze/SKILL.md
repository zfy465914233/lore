...
2. **获取PDF内容和图片**
   - 使用WebFetch获取PDF
   - **重要**：提取论文中的所有图片
   - 保存图片到`paper-notes/[领域]/[论文标题]/images/`
...
**插入规则：**
1. **架构图 / 方法图** → 插入「方法概述 / 方法框架」章节
2. **实验结果图** → 插入「实验结果」章节对应小节
3. **对比图 / 消融图** → 插入「深度分析」或「与相关论文对比」章节
4. **每张图片必须有描述性 caption**（中文），格式为 `> 图X：描述`
5. **图片编号从1开始连续递增**
6. **仅插入与论文内容直接相关的图**，跳过 logo、装饰性图标
7. **优先插入核心架构图**（通常1-2张），然后是关键实验结果图（3-5张）
8. 每篇论文笔记**至少插入3张图片**（如果源码中有的话）
...
- **图片必须嵌入笔记** - 步骤 4.4（提取图片并嵌入笔记）是必须执行的步骤，不可跳过。生成的笔记 markdown 中必须包含 `![[filename|800]]` 格式的图片引用，每篇笔记至少 3 张图（如果源码中有）
- **优雅处理错误** - 如果一个源失败则继续

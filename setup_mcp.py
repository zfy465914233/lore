#!/usr/bin/env python3
"""Inject Scholar Agent MCP configuration into the parent project.
...
This will:
  ...
  5. Add a CLAUDE.md snippet instructing the AI to use Scholar Agent tools
"""
...
def get_scholar_dir() -> Path:
    ...
    print("Error: run this script from the parent project root or the scholar-agent directory.", file=sys.stderr)
    sys.exit(1)
...
def setup_claude_md(parent_root: Path, scholar_dir: Path) -> None:
    ...
    snippet = (
        "# Scholar Agent\n"
        "\n"
        "This project has Scholar Agent for domain knowledge retrieval and research.\n"
